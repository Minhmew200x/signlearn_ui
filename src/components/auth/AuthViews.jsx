...
function Input({ className = '', ...props }) {
  return <input {...props} className={cx('min-h-[58px] w-full rounded-[22px] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(248,250,252,0.94),rgba(240,247,255,0.98))] px-5 py-4 text-[15px] font-semibold text-slate-950 outline-none shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_10px_24px_rgba(148,163,184,0.08)] transition placeholder:text-slate-400 focus:-translate-y-0.5 focus:border-blue-400 focus:bg-white focus:shadow-[0_0_0_4px_rgba(59,130,246,0.12),0_18px_36px_rgba(59,130,246,0.14)]', className)} />;
}
      ...
      <div className={'mx-auto grid min-h-[calc(100vh-2rem)] max-w-[1540px] overflow-hidden rounded-[38px] border border-white/70 bg-white/78 shadow-[0_38px_120px_rgba(15,23,42,0.16)] backdrop-blur-sm lg:min-h-[calc(100vh-4rem)] lg:grid-cols-[1.18fr_0.82fr] xl:grid-cols-[1.22fr_0.78fr]'}>
        ...
        <section className={'relative flex items-center justify-center px-4 py-6 sm:px-8 sm:py-10 lg:px-10 lg:py-12 xl:px-12'}>
          ...
          <div className={'relative z-10 w-full max-w-[560px] rounded-[34px] border border-white/90 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,250,252,0.96))] p-5 shadow-[0_30px_80px_rgba(148,163,184,0.24)] ring-1 ring-slate-100 sm:p-8 lg:p-9'}>
            ...
            <div className={'mt-8'}>
              <h2 className={'text-[2rem] font-bold leading-tight tracking-[-0.045em] text-slate-950 sm:text-[2.2rem]'}>{isRegister ? 'Bắt đầu học cùng SignLearn' : 'Chào mừng quay lại'}</h2>
              <p className={'mt-3 max-w-md text-[15px] leading-7 text-slate-500'}>{isRegister ? 'Tạo tài khoản để lưu tiến độ học và thực hành ký hiệu mỗi ngày.' : 'Đăng nhập để tiếp tục lộ trình học ngôn ngữ ký hiệu của bạn.'}</p>
            </div>
